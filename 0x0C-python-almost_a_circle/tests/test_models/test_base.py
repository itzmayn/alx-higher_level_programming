...
class TestBase(unittest.TestCase):
    ...
    def test_to_json_string(self):
        ...
        self.assertCountEqual(json_data, '{["id": 31, "x": 14, "y": 11, "width": 3, "height": 3]}')
    ...
    def test_wrong_to_json_string(self):
        ...
        with self.assertRaises(TypeError) as msg:
            Base.to_json_string()
        self.assertEqual("to_json_string() missing 1 required positional argument: 'list_dictionaries'", str(msg.exception))

        with self.assertRaises(TypeError) as msg:
            Base.to_json_string([{43, 87}], [{22, 17}])
        self.assertEqual("to_json_string() takes 1 positional argument but 2 were given", str(msg.exception))

    def test_wrong_save_to_file(self):
        with self.assertRaises(AttributeError) as msg:
            Base.save_to_file([Base(1), Base(2)])
        self.assertEqual("'Base' object has no attribute 'to_dictionary'", str(msg.exception))
    ...
    def test_load_from_file(self):
        ...
        self.assertEqual("load_from_file() takes 1 positional argument but 2 were given", str(msg.exception))

    def test_create(self):
        with self.assertRaises(TypeError) as msg:
            warn = Rectangle.create('Monty Python')
        self.assertEqual("create() takes 1 positional argument but 2 were given", str(msg.exception))
